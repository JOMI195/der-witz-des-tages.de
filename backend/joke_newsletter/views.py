...
from joke_newsletter.tasks import (
    joke_of_the_day_email_only_workflow,
...
class JokeNewsletterViewSet(viewsets.ViewSet):
    throttle_classes = [BurstRateThrottle, SustainedRateThrottle]
    ...
    @extend_schema(
        ...
        parameters=[
            OpenApiParameter(
                ...
                type=str,
                enum=["email_only", "full", "email_only_specific", "full_specific"],
            ),
            OpenApiParameter(
    ...
    @action(detail=False, methods=["post"], url_path="trigger-joke-of-the-day-workflow")
    def trigger_joke_of_the_day_workflow(self, request):
        ...
        try:
            if workflow_type == "email_only":
                joke_of_the_day_email_only_workflow.delay()
                ...
                return Response(
                    {
                        "error": "Invalid workflow type. Choose 'email_only', 'full', 'email_only_specific', or 'full_specific'."
                    },
                    status=status.HTTP_400_BAD_REQUEST,
                )
