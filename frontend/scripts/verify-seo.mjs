...
for (const path of prerendered) {
  ...
  if (/Willkommen bei "Der Witz des Tages"/.test(html)) {
    failures.push(`${relative}: old duplicated <noscript> block still present`);
  }
  // The umami tracker is injected into the served files at runtime by
  // scripts/inject-umami-tracker.sh, so it is deliberately absent here.
  if (!/<\/head>/.test(html)) {
    failures.push(`${relative}: no </head> for the tracker injection to hook into`);
  }
}
